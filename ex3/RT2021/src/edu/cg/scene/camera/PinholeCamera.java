...
public class PinholeCamera {
	...
	public PinholeCamera(Point cameraPosition, Vec towardsVec, Vec upVec, double distanceToPlain) {
		...
		resY = 200;
		planeWidth = 200;
		pixelWidth = this.planeWidth / this.resX;
	}
	...
	public void initResolution(int height, int width, double planeWidth) {
		...
		resX = width;
		resY = height;
		this.pixelWidth = planeWidth / resX;
	}
	...
	public Point transform(int x, int y) {
		...
		double vUp = (y-(int)resY*0.5);
		double vRight = (x-(int)resX*0.5);
		double upDistance = vUp * pixelWidth;
		double rightDistance = vRight * pixelWidth;
		Vec upVector = upVec.mult(upDistance).neg();
		Vec rightVector = rightVec.mult(rightDistance);
		Vec addVector = upVector.add(rightVector);
		Point centerPointPixel = centerPoint.add(addVector);
		return centerPointPixel;
	}
